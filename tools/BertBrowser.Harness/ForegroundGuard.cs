...
/// <remarks>
/// <para>
/// Showing the window does not activate it, yet a WPF window is measurably made foreground once
/// during startup — for a fraction of a second — and neither <c>WS_EX_NOACTIVATE</c>, disabling
/// the window, nor refusing WPF focus prevents it. Since the whole premise here is that the user
/// can keep typing, the answer is to watch for it and give the foreground straight back.
/// </para>
/// <para>
/// A poll rather than a hook because the activation arrives through no event this process sees.
/// Ten milliseconds is far below the interval at which a person notices a focus change, and the
/// cost is one Win32 call per tick.
/// </para>
/// </remarks>
internal sealed class ForegroundGuard : IDisposable
{
    ...
    /// <summary>Starts guarding the foreground on behalf of whoever currently holds it.</summary>
