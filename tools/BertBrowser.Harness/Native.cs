...
/// wants unsafe code, and turning that on for a project whose only native calls are a handful of
...
internal static class Native
{
    ...
    /// </summary>
    /// <remarks>
    /// Belt to <c>ShowActivated = false</c>'s braces. That alone stops the window taking focus as
    /// it opens; this stops anything taking it afterwards, including the window itself calling
    /// <c>SetForegroundWindow</c> — which is what a file list focusing itself after a load looks
    /// like from the outside, and BertBrowser's does exactly that.
    /// </remarks>
    public static void MakeNonInteractive(Window window)
    {
        ...
        // window; SetFocus from the owning thread still activates it, and DirectoryTabView focuses
        // the list whenever a listing arrives. A disabled window cannot take focus by any route,
        // and since no real keystroke is ever sent here, it costs nothing.
