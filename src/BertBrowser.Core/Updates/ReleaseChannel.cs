namespace BertBrowser.Core.Updates;
...
/// <c>1.2.3-unstable.42</c>, so the version already carries the answer — there is no second source to
/// drift from it, and unlike asking Velopack it answers for a build that was never installed.
/// </para>
/// <para>
/// The distinction is load-bearing rather than cosmetic: unstable ships as a GitHub <em>pre-release</em>
/// and stable does not, so it decides the <c>prerelease</c> flag the update source is built with. Get
/// it wrong in one direction and an unstable copy never finds its own feed; get it wrong in the other
/// and a stable copy is handed a pre-release that carries no feed for its channel.
/// </para>
/// <para>
/// It matches the <em>first identifier of the SemVer pre-release tag</em>, not a substring, so build
/// metadata (<c>1.2.3+unstable</c>) and unrelated tags (<c>1.2.3-beta.1</c>) are both stable.
/// </para>
/// </remarks>
